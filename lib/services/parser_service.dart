...
class ParserService {
  ...
  String _sanitizeHtml(String htmlContent) {
    // Remover scripts
    final scriptRegex =
        RegExp(r'(?i)<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>');
  ...
  MediaItem? _parsePathFormat(String line) {
    ...
    final downloadUrl =
        line.startsWith('http') ? line : '${Constants.baseUrl}$line';
  ...
  MediaType _detectType(String title) {
    ...
    if (RegExp(r'\d{4}.*1080p|\d{4}.*720p|bluray|dvdrip|hdrip')
        .hasMatch(lower)) {
      return MediaType.movie;
    }
  ...
  bool _isQuality(String text) {
    ...
    return RegExp(
            r'4k|2160p|1080p|720p|480p|360p|hd|sd|uhd|hdr|bluray|web-dl|webrip|hdtv')
        .hasMatch(lower);
  }
  ...
  String? _extractSizeFromHtml(String html, String fileName) {
    ...
    final pattern = RegExp(
        r'(?i)>($escapedFileName.*?)</a>.*?(\d+(?:\.\d+)?\s*(?:GB|MB|KB))');
  ...
  String _fileNameToTitle(String fileName) {
    ...
    title = title.replaceAll(
        RegExp(r'(?i)\b(1080p|720p|480p|4k|BluRay|WEB-DL|x264|x265|AAC|AC3)\b'),
        '');
